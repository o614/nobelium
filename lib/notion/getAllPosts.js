...
export async function getAllPosts ({ includePages = false }) {
  ...
  // === 诊断日志开始 ===
  console.log("============ 诺贝尔奖级诊断 ============")
  console.log("1. 从 Notion 成功抓取的原始数据总数:", data.length)
  
  if (data.length > 0) {
    console.log("2. 抽取第一篇文章的原始属性进行检查:")
    console.log(JSON.stringify(data[0], null, 2))
  } else {
    console.log("警告：从 Notion 抓回来的数据就是空的！请检查 NOTION_PAGE_ID 是否填错，或者页面是否没有内容。")
  }

  // remove all the the items doesn't meet requirements
  const posts = filterPublishedPosts({ posts: data, includePages })

  console.log("3. 经过状态(status)和类型(type)过滤后，存活的文章数:", posts.length)
  console.log("========================================")
  // === 诊断日志结束 ===
